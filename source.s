...
cmpsec:
	...
	MOVEQ R4, #gumcost
	LDREQ R5, =gumstr
	MOVEQ R6, #0 @if inputted G, set up registers for gum
	CMP R0, #'P'
	MOVEQ R4, #peanutcost
	LDREQ R5, =peanutstr
	MOVEQ R6, #4 @if inputted P, set up registers for peanuts
	CMP R0, #'C'
	MOVEQ R4, #crackercost
	LDREQ R5, =crackerstr
	MOVEQ R6, #8 @if inputted C, set up registers for crackers
	CMP R0, #'M'
	MOVEQ R4, #mandmcost
	LDREQ R5, =mandmstr
	MOVEQ R6, #12 @if inputted M, set up registers for M&Ms
	CMP R0, #'i'
...
@R
@You inputted a key for an invalid item. Returning to selection...
@Enter item selection: Gum (G), Peanuts (P), Cheese Crackers (C), or M&Ms (M):
@G
...
@Change of 10 cents has been returned.
@Enter item selection: Gum (G), Peanuts (P), Cheese Crackers (C), or M&Ms (M):
@P
...
@Change of 25 cents has been returned.
@Enter item selection: Gum (G), Peanuts (P), Cheese Crackers (C), or M&Ms (M):
@i
@Current Inventory:
@Gum: 1
